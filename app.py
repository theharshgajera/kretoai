...
client = Client(api_key=os.getenv("GEMINI_API_KEY"), http_options={'api_version': 'v1alpha'})
    ...
    try:
        ...
        for ch_id in channel_ids:
            ...
            shorts_videos = [
                ...
                if parse_duration(v.get("contentDetails", {}).get("duration", "")) <= 90
            ]

            if not shorts_videos:
                continue

            # ✅ Exclude videos newer than 3 days from avg calculation
            ...
            for v in shorts_videos:
                ...
                try:
                    ...
                    if dt < cutoff:
                        mature_shorts.append(v)
                except Exception:
                    mature_shorts.append(v)

            videos_for_avg = mature_shorts if mature_shorts else shorts_videos
            avg_recent_views = sum(
            ...
            for v in shorts_videos[:30]:
                views = int(v["statistics"].get("viewCount", 0))
                multiplier = views / avg_recent_views if avg_recent_views > 0 else 0
                if multiplier > 1:
                    trending_videos.append(v)
                if len(trending_videos) >= 15:
                    break

            def format_video(v, avg=avg_recent_views, ch_id=ch_id, ch_title=ch_title, subs=subs_count):
                video_id = v.get("id")
                snippet = v.get("snippet", {})
                views = int(v["statistics"].get("viewCount", 0))
                duration = parse_duration(v.get("contentDetails", {}).get("duration", ""))
                multiplier = round(views / avg, 2) if avg else 0
        ...
        # ✅ Fixed interleave — every channel contributes both trending and popular
        all_videos = []
        for i in range(15):
            for channel in channel_shorts:
                if i < len(channel["trending"]):
                    all_videos.append(channel["trending"][i])
                if i < len(channel["popular"]):
                    existing_ids = {v["video_id"] for v in all_videos}
                    if channel["popular"][i]["video_id"] not in existing_ids:
        ...
        for ch_id in channel_ids:
            ...
            for v in comp_videos:
                ...
                try:
                    dt = datetime.fromisoformat(published_at.replace("Z", "+00:00"))
                    if dt < cutoff:
                        mature_videos.append(v)
                except Exception:
                    mature_videos.append(v)

            videos_for_avg = mature_videos if mature_videos else comp_videos
            avg_recent_views = sum(
                int(v["statistics"].get("viewCount", 0)) for v in videos_for_avg
            ) / len(videos_for_avg)

            # ✅ popular — only videos with multiplier <= 1 (at or below average)
            popular_videos = sorted(
                [
                    v for v in comp_videos
                    if avg_recent_views > 0 and
            ...
            # ✅ trending — only videos with multiplier > 1 (above average)
            trending_videos = []
...
def similar_channels():
    """Fetch up to 15 high-quality similar channels using weighted search similarity."""
    try:
        data = request.get_json()
        ...
        # ----------------------------
        # INTERNAL RELATED FUNCTION
        # ----------------------------
        def get_related_videos_local(video_id, m=50):
            try:
                # Get video title
                ...
                app.logger.error(f"Local related fetch failed for {video_id}: {str(e)}")
                return []
        ...
        input_channel_title = channel_response['items'][0]['snippet']['title']

        # ----------------------------
        # Get 10 top videos
        # ----------------------------
        top_videos = get_top_videos(channel_id, n=10)
        if not top_videos:
            return jsonify({'error': 'No top videos found'}), 404

        channel_frequency = {}

        # ----------------------------
        # Weighted similarity logic
        # ----------------------------
        for video in top_videos:
            related_videos = get_related_videos_local(video['video_id'], m=50)

            for vid in related_videos:
                cid = vid.get('channel_id')
                if cid and cid != channel_id:
                    channel_frequency[cid] = channel_frequency.get(cid, 0) + 1

        if not channel_frequency:
            return jsonify({'error': 'No similar channels found'}), 404

        # Rank top 30 before filtering
        import heapq
        ranked_channel_ids = [
            cid for cid, _ in heapq.nlargest(
                30,
                channel_frequency.items(),
                key=lambda x: x[1]
            )
        ]
        ...
        for item in channel_details_response.get('items', []):
            ...
            similar_channels.append({
                ...
                'frequency_score': channel_frequency.get(item['id'], 0)
            })

        # Final top 15
        similar_channels = sorted(
            similar_channels,
        ...
        if not similar_channels:
            return jsonify({'error': 'No similar channels found after filtering'}), 404
