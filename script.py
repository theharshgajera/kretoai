...
#         """Process local video file to extract transcript"""
#         try:
#             # Validate video file
...
class VideoProcessor:
    ...
    def extract_transcript_details(self, youtube_video_url, max_retries=3, retry_delay=2):
        """Extract transcript using yt-dlp - works better for deployed environments and supports multiple languages"""
        self.rate_limit_wait()
        ...
        for attempt in range(max_retries):
            try:
                ...
                ydl_opts = {
                    ...
                    'subtitleslangs': ['en', 'hi', 'en-US', 'en-GB', 'hi-IN'],  # English and Hindi support
                ...
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    try:
                        ...
                        logger.info(f"Extracting video info for: {youtube_video_url}")
                        ...
                        # Priority order: manual English > manual Hindi > auto English > auto Hindi > any available
                        subtitle_priority = [
                            ('subtitles', ['en', 'en-US', 'en-GB']),
                            ('subtitles', ['hi', 'hi-IN']),
                            ('automatic_captions', ['en', 'en-US', 'en-GB']),
                            ('automatic_captions', ['hi', 'hi-IN']),
                        ]
                        
                        for subtitle_type, langs in subtitle_priority:
                            source = subtitles if subtitle_type == 'subtitles' else automatic_captions
                            
                            for lang in langs:
                                if lang in source:
                                    # Get the subtitle data
                                    subtitle_list = source[lang]
                                    for subtitle in subtitle_list:
                                        if subtitle.get('ext') == 'json3':
                                            ...
                                            if subtitle_url:
                                                try:
                                                    logger.info(f"Downloading {subtitle_type} subtitle in {lang}")
                                                    with urllib.request.urlopen(subtitle_url, timeout=10) as response:
                                                        ...
                                                        if len(transcript_text) >= 50:
                                                            used_language = lang
                                                            logger.info(f"Successfully extracted {subtitle_type} transcript in {lang} ({len(transcript_text)} chars)")
                                                            break
                                                except Exception as e:
                                                    logger.warning(f"Failed to download subtitle from {subtitle_url}: {e}")
                                                    continue
                        ...
                        if not transcript_text or len(transcript_text) < 50:
                            logger.info("Trying any available subtitle language...")
                            all_sources = [(subtitles, 'manual'), (automatic_captions, 'auto')]
                            
                            for source, source_name in all_sources:
                                for lang_code in source.keys():
                                    subtitle_list = source[lang_code]
                                    for subtitle in subtitle_list:
                                        if subtitle.get('ext') == 'json3':
                                            subtitle_url = subtitle.get('url')
                                            if subtitle_url:
                                                try:
                                                    logger.info(f"Trying {source_name} subtitle in {lang_code}")
                                                    with urllib.request.urlopen(subtitle_url, timeout=10) as response:
                                                        subtitle_data = json.loads(response.read().decode('utf-8'))
                                                        events = subtitle_data.get('events', [])
                                                        text_segments = []
                                                        ...
                                                        if len(transcript_text) >= 50:
                                                            ...
                                                            logger.info(f"Successfully extracted {source_name} transcript in {lang_code}")
                                                            break
                                                except Exception as e:
                                                    logger.warning(f"Failed with {lang_code}: {e}")
                                                    continue
                        ...
                        # If no subtitles found at all
                        if not transcript_text or len(transcript_text) < 50:
                            # Check if video has any subtitles at all
                            if not subtitles and not automatic_captions:
                                return {"error": "No transcripts/subtitles available for this video", "transcript": None, "stats": None}
                            else:
                                return {"error": "Could not extract readable transcript from available subtitles", "transcript": None, "stats": None}
                        ...
                        stats = self._calculate_transcript_stats(transcript_text)
                        stats['language'] = used_language
                        logger.info(f"Transcript extraction successful - {stats['char_count']} characters, {stats['word_count']} words in {used_language}")
                        ...
                        if 'private' in error_msg or 'unavailable' in error_msg:
                            ...
                            return {"error": "Video requires authentication or age verification", "transcript": None, "stats": None}
                        elif 'copyright' in error_msg:
                            return {"error": "Video has copyright restrictions", "transcript": None, "stats": None}
                        else:
                            if attempt == max_retries - 1:
                                return {"error": f"Error accessing video: {str(e)}", "transcript": None, "stats": None}
                            continue
                ...
                if 'http error 429' in error_msg or 'too many requests' in error_msg:
                    if attempt < max_retries - 1:
                        ...
                        return {"error": "Rate limit exceeded. Please try again in a few minutes", "transcript": None, "stats": None}
